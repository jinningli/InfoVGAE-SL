...
parser.add_argument('--baseline_embedding', type=str, default=None, help='baseline_embedding to use')
...
parser.add_argument('--kthreshold', type=int, default=1, help='minimum keyword count to keep the sample')
parser.add_argument('--uthreshold', type=int, default=10, help='minimum user tweet count to keep the sample')
parser.add_argument('--tthreshold', type=int, default=20, help='minimum user tweet count to keep the sample')
...
# Journal Version Debug
parser.add_argument('--pos_weight_lambda', type=float, default=1.0, help='Lambda for positive sample weight')
parser.add_argument('--smooth_matrix', action="store_true", help='Lambda for positive sample weight')
parser.add_argument('--beta', type=float, default=0.0, help='beta for new loss funcion')
parser.add_argument('--decode_mode', type=str, default=None, help='beta for new loss funcion')
parser.add_argument('--turning_epoch', type=int, default=-1, help='epoch to apply beta')
parser.add_argument('--score_save_path', type=str, default=None, help='path to save best score')

# For Discriminator
parser.add_argument('--gamma', type=float, default=1e-3, help='weight for tc loss')
parser.add_argument('--n_init', type=int, default=10, help='n_init')
...
# Prepare dataset
if args.config_name is not None and args.config_name.find("_bill_") != -1:
    # Use this for bill
    ...
    dataset = TwitterDataset(csv_path=args.data_path,
                             ...
                             uthreshold=args.uthreshold, tthreshold=args.tthreshold, num_process=args.num_process,
...
evaluator.init_from_value(trainer.result_embedding, dataset.user_label, dataset.asser_label,
                          ...
                          output_dir=args.output_path, user_label_numerical=dataset.user_label_numerical)
# evaluator.plot(show=True, save=True, tag=args.dataset)
